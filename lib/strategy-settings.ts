...
export const strategyLabSettingsSchema = z.object({
  ...
  gapTrading: z.object({
    // This engine is intentionally analytical-only until an event-level feed,
    // trade-flow confirmation and out-of-sample calibration are available.
    ...
    sampleWindowMs: z.coerce.number().int().min(2_000).max(120_000).default(20_000),
    minPersistenceMs: z.coerce.number().int().min(0).max(60_000).default(3_000),
    maxPersistenceMs: z.coerce.number().int().min(2_000).max(120_000).default(15_000),
